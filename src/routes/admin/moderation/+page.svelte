<script lang="ts">
  ...
  import { createModerationDownload, loadModerationDetail, loadModerationQueue, loadModerationVerification, moderationQueue, setModerationStatus } from '$lib/stores/admin';
  ...
  async function choose(item: ModerationItem) {
    ...
    detailLoading = true;
    try {
      const loaded=await loadModerationDetail(item.id);
      if(request!==detailRequest)return;
      detail=loaded;
      note = loaded.moderationNotes ?? item.notes ?? '';
      detailLoading = false;
      if(loaded.version)void verifySelected(item.id,request);
    } catch (error) {
      if(request!==detailRequest)return;
  ...
  async function verifySelected(id:string,request:number) {
    verificationLoading=true;
    verificationError='';
    try {
      const result=await loadModerationVerification(id);
      if(request!==detailRequest || !detail?.version)return;
      detail={...detail,version:{...detail.version,...result}};
    } catch(error) {
      if(request!==detailRequest)return;
      verificationError=error instanceof Error?error.message:'Package verification could not be completed.';
    } finally {
      if(request===detailRequest)verificationLoading=false;
    }
  }
  ...
  onMount(()=>{void loadModerationQueue(true).catch(()=>{});});
  ...
  <section class="admin-panel glass workspace">
    ...
    {:else if detail}
      ...
      <section class="review-section">
        ...
        {#if detail.version}
          <div class="file-card">
            <div class:checking={detail.version.packageVerified===null} class:failed={detail.version.packageVerified===false}><Icon name={detail.version.packageVerified===null?'refresh':detail.version.packageVerified?'check':'alert'} size={20}/></div>
            <span><b>{detail.version.packageName}</b><small>{detail.version.size} · Version {detail.version.version} · {detail.version.packageVerified===null?(verificationLoading?'Checking R2 object…':'Verification pending'):detail.version.packageVerified?'Object verified':'Object could not be verified'}</small></span>
          ...
          {#if verificationError}<div class="inline-warning"><Icon name="alert" size={20}/><span><b>Live package verification failed</b><small>{verificationError}</small></span><button class="admin-mini-button" type="button" onclick={()=>selected&&verifySelected(selected.id,detailRequest)}>Retry</button></div>{/if}
          {#if detail.version.documentationName}<div class="file-card"><div class:checking={detail.version.documentationVerified===null} class:failed={detail.version.documentationVerified===false}><Icon name={detail.version.documentationVerified===null?'refresh':detail.version.documentationVerified===false?'alert':'check'} size={20}/></div><span><b>{detail.version.documentationName}</b><small>{detail.version.documentationVerified===null?(verificationLoading?'Checking documentation…':'Verification pending'):detail.version.documentationVerified===false?'Documentation could not be verified':'Documentation available'}</small></span><button class="admin-mini-button" type="button" disabled={Boolean(downloadBusy)} onclick={()=>download('documentation')}>Download guide</button></div>{/if}
      ...
      <section class="decision-section">
        ...
        <div class="review-actions">
          ...
          <button class="button approve-action" type="button" disabled={actionBusy || detail.version?.packageVerified!==true || detail.images.length<3} onclick={()=>decide('Approved')}><Icon name="check" size={18}/>Approve and publish</button>
        </div>
        {#if detail.version?.packageVerified!==true || detail.images.length<3}<div class="approval-block"><Icon name="lock" size={17}/><span>{verificationLoading?'Approval will unlock as soon as the live package check completes.':'Approval is blocked until the package verifies and at least three preview images are present.'}</span></div>{/if}
      </section>
    {:else}
