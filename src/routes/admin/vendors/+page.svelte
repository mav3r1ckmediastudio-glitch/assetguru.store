<script lang="ts">
  ...
  import { setVendorStatus, vendorApplications } from '$lib/stores/admin';
  ...
  function choose(v:VendorApplication){ selected=v; reason=v.reason ?? ''; }
  function decide(status:VendorStatus){ if(!selected) return; setVendorStatus(selected.id,status,reason); selected={...selected,status,reason:reason || selected.reason}; }
</script>
...
<header class="admin-page-head">
  ...
  <div class="admin-head-actions"><a class="button button-secondary" href="/admin/settings"><Icon name="sliders" size={18}/> Vendor rules</a></div>
</header>
...
<div class="vendor-layout">
  <section class="admin-panel glass list-panel">
    ...
    <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Vendor</th><th>Country</th><th>Stripe</th><th>Risk</th><th>Assets</th><th>Status</th><th></th></tr></thead><tbody>
      ...
      {:else}<tr><td colspan="7"><div class="admin-empty"><Icon name="store" size={28}/><strong>No matching vendors</strong></div></td></tr>{/each}
    </tbody></table></div>
  </section>
  ...
  <aside class="admin-panel glass detail">
    {#if selected}
      ...
      <div class="facts"><span><b>{selected.products}</b><small>submitted products</small></span><span><b>{selected.status}</b><small>application status</small></span><span><b>{selected.stripe}</b><small>Stripe onboarding</small></span></div>
