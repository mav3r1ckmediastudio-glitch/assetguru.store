...
{#if page.url.searchParams.get('creator')==='unavailable'}
  <div class="account-notice"><Icon name="alert" size={20}/><span><b>This email is registered as a buyer account.</b><small>Open the buyer area below, or sign out and use the email attached to your creator storefront.</small></span><a href="/auth/logout">Use another account</a></div>
{/if}
...
<style>
  .account-notice{margin-bottom:14px;padding:14px 16px;display:grid;grid-template-columns:auto 1fr auto;gap:11px;align-items:center;border:1px solid rgb(255 181 71/.4);border-radius:12px;color:#ffb547;background:rgb(255 181 71/.07)}.account-notice span{display:grid}.account-notice b{color:#f5f8ff}.account-notice small{margin-top:4px;color:#aab5c8;font-size:9px}.account-notice a{color:#00e5ff;font-size:9px;font-weight:800}
