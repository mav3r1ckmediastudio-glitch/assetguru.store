...
export async function PATCH({ locals, request, params }: import('./$types').RequestEvent) {
  try {
    ...
    const { data: product, error: productError } = await admin
      .from('products')
      .select(`*,category:categories(name),images:product_images(storage_path,sort_order),versions:product_versions(id,version,is_current,status,package_path,documentation_path,file_size_bytes,release_notes,created_at)`)
    ...
    if (effectiveExtended < effectivePrice) {
      return json({ message: 'Extended licence price cannot be below the standard price.' }, { status: 400 });
    }

    const effectiveCategoryName = body.category ?? product.category?.name ?? '';
    const effectiveSubcategory = body.subcategory ?? product.subcategory ?? '';
    ...
    if (body.category !== undefined || body.subcategory !== undefined) {
      const taxonomy = taxonomyCategory(effectiveCategoryName);
      if (!taxonomy) return json({ message: 'Choose a valid AssetGuru category.' }, { status: 400 });
      if (effectiveSubcategory && !taxonomy.subcategories.includes(effectiveSubcategory)) {
    ...
    if (body.price !== undefined) patch.price_pence = effectivePrice;
    if (body.extendedPrice !== undefined) patch.extended_price_pence = effectiveExtended;
    if (body.category !== undefined) patch.category_id = categoryId;
    ...
    if (body.maxVersion !== undefined) patch.max_version = body.maxVersion;
    if (body.sourceFiles !== undefined) patch.source_files = body.sourceFiles;
    if (body.dependencies !== undefined) patch.dependencies = body.dependencies;
    if (body.performance !== undefined) patch.performance = body.performance;
    if (body.features !== undefined) patch.features = body.features;
    if (body.contents !== undefined) patch.contents = body.contents;
    if (body.tags !== undefined) patch.tags = body.tags;
    if (body.formats !== undefined) patch.formats = body.formats;
    if (body.licence !== undefined) patch.licence = body.licence;
    if (body.showcaseVideoUrl !== undefined) patch.showcase_video_url = showcaseVideo?.canonicalUrl ?? null;

    if (body.status) {
      const next = statusMap[body.status];
      if (next === 'published' && product.status !== 'published') {
        return json({ message: 'Only an administrator can publish a product.' }, { status: 403 });
      }
      if (next === 'in_review') {
        if (vendor.status !== 'approved') {
          return json({ message: 'Your creator account must be approved before submitting products for review.' }, { status: 403 });
        }
        if (effectivePrice === 0 && !settings?.allow_free_assets) {
          return json({ message: 'Free assets are currently disabled.' }, { status: 400 });
        }
        if (effectivePrice > 0 && effectivePrice < Number(settings?.minimum_price_pence ?? 0)) {
          return json({ message: `The minimum product price is £${(Number(settings?.minimum_price_pence ?? 0) / 100).toFixed(2)}.` }, { status: 400 });
        }
        ...
        const taxonomy = taxonomyCategory(effectiveCategoryName);
        if (
          effectiveTitle.trim().length < 5 ||
          effectiveSummary.trim().length < 20 ||
          effectiveDescription.trim().length < 60 ||
          !taxonomy ||
          !taxonomy.subcategories.includes(effectiveSubcategory)
        ) {
          return json({ message: 'Complete the title, category, subcategory, summary and description before submitting for review.' }, { status: 409 });
        }

        const latest = [...(product.versions ?? [])]
          .sort((a: any, b: any) => String(b.created_at).localeCompare(String(a.created_at)))[0];
        if (!latest || !(await packageObjectExists(latest.package_path, Number(latest.file_size_bytes)))) {
          return json({ message: 'Complete the asset package upload before submitting for review.' }, { status: 409 });
        }
        if (latest.documentation_path && !(await packageObjectExists(latest.documentation_path))) {
          return json({ message: 'The documentation upload is incomplete.' }, { status: 409 });
        }
        if ((product.images ?? []).length < 3) {
          return json({ message: 'Upload at least three preview images before submitting for review.' }, { status: 409 });
        }
        for (const image of product.images ?? []) {
          if (!(await legacyObjectExists('product-images', image.storage_path))) {
            return json({ message: 'One or more preview uploads are incomplete.' }, { status: 409 });
          }
        }
      }
      patch.status = next;
    ...
    const { data: updated, error } = await admin
      ...
      .update(patch)
      .eq('id', product.id)
      .select(`*,category:categories(name),images:product_images(storage_path,sort_order),versions:product_versions(id,version,is_current,status,file_size_bytes,release_notes,created_at)`)
      .single();
    if (error) throw error;

    if (body.version) {
      const latest = [...(updated.versions ?? [])]
        .sort((a: any, b: any) => String(b.created_at).localeCompare(String(a.created_at)))[0];
      if (latest) {
        const { error: versionError } = await admin.from('product_versions').update({ version: body.version }).eq('id', latest.id);
    ...
    const imageUrl = sortedImages[0]
      ? admin.storage.from('product-images').getPublicUrl(sortedImages[0].storage_path).data.publicUrl
      : '/images/marketplace-grid.webp';
    ...
    return json({
      product: {
        ...
        title: updated.title,
        image: imageUrl,
        images: sortedImages.map((entry: any) => admin.storage.from('product-images').getPublicUrl(entry.storage_path).data.publicUrl),
        category: updated.category?.name ?? 'Uncategorised',
        subcategory: updated.subcategory ?? '',
        status: displayStatus(updated.status),
        price: updated.price_pence / 100,
        extendedPrice: updated.extended_price_pence == null ? undefined : updated.extended_price_pence / 100,
        summary: updated.summary,
        description: updated.description,
        compatibility: updated.compatibility,
        maxVersion: updated.max_version,
        ...
        showcaseVideoUrl: updated.showcase_video_url || undefined,
        sales: updated.sales_count,
        revenue: 0,
        views: updated.view_count,
        conversion: 0,
        rating: Number(updated.rating_average),
        reviews: updated.review_count,
        version: body.version ?? latest?.version ?? '1.0.0',
        versions: sortedVersions.map((entry: any) => ({
          id: entry.id,
          version: entry.version,
          status: entry.status,
          isCurrent: Boolean(entry.is_current),
          size: entry.file_size_bytes ? `${Math.round(entry.file_size_bytes / 1024 / 1024)} MB` : '0 MB',
          created: new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(entry.created_at)),
          releaseNotes: entry.release_notes ?? ''
        })),
        updated: 'Just now',
        moderationNote: updated.moderation_notes || undefined
      }
    });
  } catch (error) {
...
export async function DELETE({ locals, request, params }: import('./$types').RequestEvent) {
  try {
    ...
    const { data: product } = await admin
      .from('products')
      .select(`id,status,sales_count,images:product_images(storage_path),versions:product_versions(package_path,documentation_path)`)
      .eq('vendor_id', vendor.id)
      .eq('slug', params.slug)
      .single();
    if (!product) return json({ message: 'Product not found.' }, { status: 404 });
    ...
    if (imagePaths.length) {
      const { error } = await admin.storage.from('product-images').remove(imagePaths);
      if (error) throw error;
    }
