<script lang="ts">
  ...
  async function deleteDraft() {
    if (!product || !confirm(`Delete the draft “${product.title}”? Uploaded files will also be removed.`)) return;
    await removeCreatorProduct(product.slug);
...
{#if product}
  ...
  <div class="editor-layout">
    <main oninput={scheduleEditorSave} onchange={scheduleEditorSave}>
      ...
      {:else}
        <section class="panel glass"><div class="panel-title"><div><span class="eyebrow">After-sale experience</span><h2>Support and buyer guidance</h2><p>These policies are shared from the creator storefront so buyers receive consistent support information.</p></div></div><div class="support-summary"><span><small>Store response target</small><b>{$storefront.responseTime || 'Not configured'}</b></span><span><small>Support email</small><b>{$storefront.supportEmail || 'Not configured'}</b></span></div><label>Store support promise<textarea rows="6" readonly value={$storefront.supportPromise || 'No support promise has been published.'}></textarea></label><label>Update commitment<textarea rows="6" readonly value={$storefront.updateCommitment || 'No update commitment has been published.'}></textarea></label><a class="button button-secondary" href="/creator/storefront"><Icon name="settings" size={17}/>Edit storefront policies</a></section>
      {/if}
    </main>
    ...
    <aside>
      ...
      <section class="submit-card glass"><h3>Product actions</h3>{#if product.status !== 'Published' && product.status !== 'In review'}<button class="button button-primary" type="button" disabled={!canSubmitReview || reviewSubmitting || saving} onclick={submitReview}>{reviewSubmitting ? 'Submitting…' : product.status === 'Changes required' ? 'Resubmit for admin review' : 'Submit for admin review'}</button>{#if missingSubmissionChecks.length}<p>Next required item: <button type="button" onclick={() => tab = missingSubmissionChecks[0].tab}>{missingSubmissionChecks[0].label}</button></p>{/if}{/if}<button class="button button-secondary" type="button" disabled={saving || editingLocked} onclick={save}>{saving ? 'Saving…' : 'Save changes'}</button>{#if product.status === 'Published'}<button class="secondary-action" type="button" onclick={() => setProductStatus(product.slug,'Retired')}><Icon name="minus" size={16}/>Retire from sale</button>{/if}{#if product.status === 'Draft' || product.status === 'Changes required'}<button class="secondary-action destructive" type="button" onclick={deleteDraft}><Icon name="trash" size={16}/>Delete draft and files</button>{/if}</section>
