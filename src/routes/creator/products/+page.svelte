<script lang="ts">
  ...
  import { creatorProducts, removeCreatorProduct, setProductStatus } from '$lib/stores/creator';
  ...
  function statusCount(value: ProductStatus | 'All') { return value === 'All' ? $creatorProducts.length : $creatorProducts.filter((item) => item.status === value).length; }

  $: filtered = $creatorProducts.filter((product) => {
    const q = search.toLowerCase();
    return (status === 'All' || product.status === status) && (!q || `${product.title} ${product.category}`.toLowerCase().includes(q));
  }).sort((a,b) => sort === 'Revenue' ? b.revenue-a.revenue : sort === 'Sales' ? b.sales-a.sales : a.title.localeCompare(b.title));

  function toggle(slug:string){ selected = selected.includes(slug) ? selected.filter((item)=>item!==slug) : [...selected,slug]; }
  function bulkStatus(next:ProductStatus){ selected.forEach((slug)=>setProductStatus(slug,next)); selected=[]; }
</script>
  ...
  <div class="product-table glass">
    ...
    {#each filtered as product}
      <article>
        ...
        <a class="product" href={`/creator/products/${product.slug}`}><img src={product.image} alt=""/><span><b>{product.title}</b><small>{product.category} · v{product.version}</small>{#if product.moderationNote}<em><Icon name="alert" size={12}/> Action required</em>{/if}</span></a>
        ...
        <div class="row-actions"><a href={`/marketplace/${product.slug}`} title="View public listing"><Icon name="eye" size={16}/></a><a href={`/creator/products/${product.slug}`} title="Edit"><Icon name="sliders" size={16}/></a>{#if product.status === 'Draft'}<button type="button" title="Delete draft" onclick={() => removeCreatorProduct(product.slug)}><Icon name="trash" size={16}/></button>{/if}</div>
      </article>
    {/each}
  </div>
{:else}
  <div class="card-grid">
    {#each filtered as product}<article class="product-card glass"><div class="image"><img src={product.image} alt=""/><StatusPill status={product.status}/></div><div class="card-copy"><small>{product.category}</small><h2>{product.title}</h2><p>v{product.version} · Updated {product.updated}</p><div class="card-metrics"><span><b>{product.sales.toLocaleString('en-GB')}</b><small>sales</small></span><span><b>£{product.revenue.toLocaleString('en-GB',{maximumFractionDigits:0})}</b><small>revenue</small></span><span><b>{product.rating || '—'}</b><small>rating</small></span></div><a class="button button-secondary" href={`/creator/products/${product.slug}`}>Manage product</a></div></article>{/each}
  </div>
{/if}

{#if !filtered.length}<div class="empty glass"><Icon name="package" size={38}/><h2>No products found.</h2><p>Try another status or search term.</p></div>{/if}
