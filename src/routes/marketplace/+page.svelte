<script lang="ts">
  ...
  $: filtered = $assets.filter((asset) => {
    ...
    const haystack = [asset.title, asset.category, asset.subcategory, asset.creator, asset.summary, ...asset.tags].join(' ').toLowerCase();
    const matchesSearch = !q || haystack.includes(q);
    const matchesCategory = selectedCategories.length === 0 || selectedCategories.includes(asset.category);
    const matchesPrice = price === 'Any price'
      || (price === 'Free' && asset.price === 0)
      || (price === 'Under £15' && asset.price > 0 && asset.price < 15)
      || (price === '£15–£25' && asset.price >= 15 && asset.price <= 25)
      || (price === '£25+' && asset.price > 25);
    const matchesRating = rating === 'Any rating' || asset.rating >= Number(rating.split('+')[0]);
    const matchesVersion = version === 'Any MAX version' || asset.maxVersion === version;
    return matchesSearch && matchesCategory && matchesPrice && matchesRating && matchesVersion
      && (!sourceFilesOnly || asset.sourceFiles)
      && (!saleOnly || Boolean(asset.oldPrice));
  });

  $: sorted = [...filtered].sort((a, b) => {
    if (sort === 'Newest') return Date.parse(b.updated) - Date.parse(a.updated);
  ...
  $: averageRating = $assets.reduce((sum, asset) => sum + asset.rating, 0) / ($assets.filter((asset) => asset.rating > 0).length || 1);
...
<section class="browse-hero">
  <div class="content-wrap">
    <div class="hero-line">
      ...
      <div class="market-stats glass"><span><b>{$assets.length}</b><small>live listings</small></span><span><b>{$categories.length}</b><small>asset categories</small></span><span><b>{averageRating ? averageRating.toFixed(1) : '—'}</b><small>average rating</small></span></div>
    </div>
      ...
      <fieldset>
        ...
        {#each $categories as category}
          <label>
            ...
            <span>{category.name}</span><small>{$assets.filter((asset) => asset.category === category.name).length}</small>
          </label>
        {/each}
      </fieldset>
    ...
    <div class="results">
      <div class="results-bar glass">
        <div class="result-count">
          ...
          <div><strong>{sorted.length} assets</strong><small>{search ? ` matching “${search}”` : ' ready to explore'}</small></div>
        </div>
      ...
      {#if sorted.length}
        <div class:listView={view === 'list'} class="asset-grid">
          {#each sorted as asset}<AssetCard {asset} {view}/>{/each}
        </div>
      {:else}
