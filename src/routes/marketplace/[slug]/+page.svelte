<script lang="ts">
  ...
  $: if (asset && !asset.gallery.includes(selectedMedia)) selectedMedia = asset.image;
  ...
  <div class="content-wrap">
    ...
    <div class="product-grid">
      <div class="media-panel">
        <div class="main-media glass"><img src={selectedMedia} alt={asset.title}/><div class="media-overlay"><span>{asset.badge ?? asset.subcategory}</span><b>{asset.compatibility}</b></div></div>
        <div class="thumbs">
          {#each asset.gallery as media, index}
            <button class:active={selectedMedia === media} type="button" onclick={() => selectedMedia = media}><img src={media} alt={`${asset.title} preview ${index + 1}`}/></button>
          {/each}
        </div>
...
<style>
  .breadcrumbs{margin-bottom:20px;display:flex;align-items:center;gap:8px;color:#718096;font-size:11px}.breadcrumbs a:hover{color:#00e5ff}.breadcrumbs em{color:#aab5c8;font-style:normal;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.product-grid{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(350px,.55fr);gap:18px}.media-panel{min-width:0}.main-media{position:relative;padding:12px;border-radius:17px;overflow:hidden}.main-media>img{width:100%;aspect-ratio:16/9;object-fit:cover;border-radius:11px}.media-overlay{position:absolute;right:24px;bottom:24px;left:24px;display:flex;justify-content:space-between;align-items:center;pointer-events:none}.media-overlay span,.media-overlay b{padding:7px 10px;border:1px solid #27547a;border-radius:8px;background:rgb(2 4 13/.78);backdrop-filter:blur(10px);font-size:9px;text-transform:uppercase;letter-spacing:.08em}.media-overlay span{color:#00e5ff}.thumbs{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-top:10px}.thumbs button{padding:4px;border:1px solid #183352;border-radius:10px;background:#050a16;cursor:pointer}.thumbs button.active{border-color:#00e5ff;box-shadow:0 0 18px rgb(0 229 255/.16)}.thumbs img{width:100%;aspect-ratio:16/9;object-fit:cover;border-radius:6px}.media-note{margin-top:11px;padding:11px 13px;display:flex;gap:9px;align-items:center;color:#718096;font-size:10px}.media-note :global(svg){color:#8b5cf6;flex:0 0 auto}
