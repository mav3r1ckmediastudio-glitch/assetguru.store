...
export async function loadAdmin(supabase:SupabaseClient<any>) {
  ...
  const [{data:products},{data:vendors},{data:cases},{data:categories},{data:allProducts},{data:settings},{data:audit},{data:orders},{data:reportItems},{data:publishedReviews},{data:downloadEvents}] = await Promise.all([
    ...
    supabase.from('vendor_profiles').select(`*, owner:profiles(display_name,email,country)`).order('created_at',{ascending:false}),
