      ...
      {#each files as file, index}
        <article class:cover={index === 0}>
          ...
          <div class="image-actions">
            {#if index > 0}<button type="button" title="Make cover image" onclick={() => makeCover(index)}>Cover</button>{/if}
            <button type="button" title="Move left" disabled={index === 0} onclick={() => move(index, -1)}>←</button>
            <button type="button" title="Move right" disabled={index === files.length - 1} onclick={() => move(index, 1)}>→</button>
            <button class="remove" type="button" title="Remove image" onclick={() => remove(index)}>×</button>
          </div>
        </article>
      {/each}
    </div>
