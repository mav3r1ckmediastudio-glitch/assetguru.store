...
export type VendorApplication = { id:string; databaseId?:string; name:string; owner:string; handle:string; email:string; country:string; submitted:string; appliedAt:string; approvedAt?:string; portfolio:string; products:string; status:VendorStatus; stripe:string; risk:'Low'|'Medium'|'High'; reason?:string; };
