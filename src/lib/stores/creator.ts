...
export async function removeCreatorProduct(slug:string){try{await apiRequest(`/api/vendor/products/${slug}`,{method:'DELETE'});creatorProducts.update((items)=>items.filter((item)=>item.slug!==slug));void loadCreatorData(true);showToast('Draft removed','info');}catch(error){showToast(error instanceof Error?error.message:'Draft could not be removed','warning');}}
