...
export async function setProductStatus(slug:string,status:ProductStatus){try{await apiRequest(`/api/vendor/products/${slug}`,{method:'PATCH',body:JSON.stringify({status})});creatorProducts.update(items=>items.map(i=>i.slug===slug?{...i,status,updated:'Just now'}:i));showToast(`Product moved to ${status.toLowerCase()}`,'success');return true;}catch(e){showToast(e instanceof Error?e.message:'Product could not be updated','warning');return false;}}
export async function updateCreatorProduct(slug:string,patch:Partial<CreatorProduct>){try{const d=await apiRequest<{product:CreatorProduct}>(`/api/vendor/products/${slug}`,{method:'PATCH',body:JSON.stringify(patch)});creatorProducts.update(items=>items.map(i=>i.slug===slug?{...i,...d.product}:i));showToast('Product changes saved','success');return d.product;}catch(e){showToast(e instanceof Error?e.message:'Product could not be updated','warning');return null;}}
